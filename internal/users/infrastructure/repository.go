...
func (r *repository) FindByID(ctx context.Context, id uuid.UUID) (entities.UserEntity, error) {
	...
	query := `
		SELECT id, email_hash, username_hash, phone_number_hash, encrypted_email, encrypted_username, encrypted_first_name, encrypted_last_name, encrypted_phone_number, encrypted_roles, encrypted_about, encrypted_introduction, image, encrypted_user_key, key_version, created_at, updated_at
		FROM users
		WHERE id = $1
	`
	...
	if err := r.db.
		...
		Scan(&e.ID, &e.EmailHash, &e.UsernameHash, &e.PhoneNumberHash, &e.EncryptedEmail, &e.EncryptedUsername, &e.EncryptedFirstName, &e.EncryptedLastName, &e.EncryptedPhoneNumber, &e.EncryptedRoles, &e.EncryptedAbout, &e.EncryptedIntroduction, &e.Image, &e.EncryptedUserKey, &e.KeyVersion, &e.CreatedAt, &e.UpdatedAt); err != nil {
